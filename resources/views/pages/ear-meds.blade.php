...
@section('content')
    @include('layouts.navbars.auth.topnav', ['title' => 'Inventory / Ear Meds'])
    <div class="container-fluid py-4">
        <div class="row mt-4">
            <div class="col-lg-12 mb-lg-0 mb-4">
                <div class="card z-index-2 h-100" style="background-color: transparent; border: none; box-shadow: none;">
                    <div class="col-lg-12 col-md-12 d-flex justify-content-end">
                        <button class="btn bg-gradient-info z-index-2 me-2" data-bs-toggle="modal" data-bs-target="#filterEarMeds">Filter</button>
                        <button class="btn bg-gradient-info z-index-2 me-2" data-bs-toggle="modal" data-bs-target="#exportEarMeds">Generate Report</button>
                        <button type="button" class="btn bg-gradient-success z-index-2" data-bs-toggle="modal" data-bs-target="#addEarMeds">Add Ear Meds</button>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-6 col-md-6">
                <form action="{{route('ear-meds.index')}}" method="GET">
                                ...
                                @forelse ($earMeds as $index => $row)
                                    <tr class="text-center">
                                        ...
                                        <td class="align-middle">
                                            <input type="hidden" id="ear-meds-details-{{$row->id}}" data-detail="{{ $row }}">
                                            <button 
                                                type="button" 
                                                class="btn bg-gradient-warning z-index-2" 
                                                data-bs-toggle="modal" 
                                                data-bs-target="#editEarMeds" 
                                                onclick = "editEarMeds('{{$row->id}}')">
                                                Edit
                                            </button>
                                            <button 
                                                type="button" 
                                                ...
                                                data-url="{{ route('ear-meds.destroy', $row->id) }}"
                                                onclick = "deleteEarMeds(this)">
                                                Delete
                                            </button>
                                        </td>
                                    </tr>
                        ...
                        <div class="table-pagination p-5">
                            <div class="row">
                                <div class="row col-sm-12 col-md-12 col-lg-12 font-weight-600"">
                                    {{$earMeds->appends(['search' => isset($requestData->search) ? $requestData->search : null])->links('components.pagination')}}
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    @include('modals.inventory.ear-meds.create')
    @include('modals.inventory.ear-meds.edit')
    @include('modals.inventory.ear-meds.filter')
...
@push('js')
    <script>
        ...
        function editEarMeds(id) {
            const detail = $(`#ear-meds-details-${id}`).data().detail;     
